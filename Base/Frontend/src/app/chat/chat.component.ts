...
interface ChatResponse {
  ...
  data: {
    ...
    session_id: number;
    user_id: number;
    claude_sonnet_4_enabled: boolean;
  };
}
...
@Component({
  ...
  template: `
    <div class="chat-container">
      <div class="chat-header">
        <div class="header-content">
          <div class="logo-section">
            ...
            <span class="claude-badge" *ngIf="claudeSonnet4Enabled">Claude Sonnet 4</span>
          </div>
          <div class="tools">
            <!-- Tool Icons as requested -->
            <button class="tool-btn" (click)="toggleTools()" [class.active]="showTools" title="Tools">
              <span class="material-icons">build</span>
            </button>
            <button class="tool-btn" (click)="toggleHistory()" [class.active]="showHistory" title="History">
              <span class="material-icons">history</span>
            </button>
            <button class="tool-btn" (click)="toggleSQLMode()" [class.active]="sqlMode" title="SQL Mode">
              <span class="material-icons">storage</span>
            </button>
            <button class="tool-btn" (click)="createNewSession()" title="New Chat">
              <span class="material-icons">add_circle</span>
              New
            </button>
            <button class="tool-btn" (click)="clearChat()" title="Clear Chat">
      ...
      <!-- Tools Panel -->
      <div class="tools-panel" *ngIf="showTools">
        <div class="tool-section">
          <h4>AI Settings</h4>
          <div class="setting-item">
            <label>Enable Claude Sonnet 4:</label>
            <input type="checkbox" [(ngModel)]="claudeSonnet4Enabled" (change)="updateClaudeSetting()">
          </div>
          <div class="setting-item">
            <label>SQL Support:</label>
            <input type="checkbox" [(ngModel)]="sqlMode">
          </div>
        </div>
        <div class="tool-section">
          <h4>Database</h4>
          <div class="db-status">
            <span class="status-indicator" [class.connected]="dbConnected"></span>
...
export class ChatComponent implements OnInit, AfterViewChecked {
  ...
  promptSuggestions: string[] = [
    ...
    "What are the benefits of using Claude Sonnet 4?",
    "How can I optimize my SQL queries?",
    "Create a chat session for me"
  ];
  ...
  sendMessage() {
    ...
    const payload = { 
      message: messageText, 
      message_type: this.sqlMode ? 'sql' : 'text',
      session_id: this.currentSessionId,
      user_email: 'user@example.com',
      user_name: 'Demo User',
      metadata: {
        sql_mode: this.sqlMode,
        claude_sonnet_4: this.claudeSonnet4Enabled
      }
    };

    this.http.post<ChatResponse>('http://localhost:3001' + endpoint, payload)
      .subscribe({
        next: (response) => {
          // Update current session ID if returned
          if (response.data.session_id && !this.currentSessionId) {
          ...
          const botMessage: Message = {
            ...
            text: response.data.message.response || 'No response received',
            response: response.data.message.response,
            isUser: false,
            timestamp: new Date(response.data.message.created_at),
            isSQL: this.sqlMode,
            tokens_used: response.data.message.tokens_used,
            response_time: response.data.message.response_time
          };
  ...
  updateClaudeSetting() {
    // Update Claude Sonnet 4 setting in backend
    this.http.put('http://localhost:3001/api/settings/enable_claude_sonnet_4', {
      value: this.claudeSonnet4Enabled.toString()
    }).subscribe({
      next: (response) => {
        console.log('Claude Sonnet 4 setting updated:', this.claudeSonnet4Enabled);
        const statusMessage: Message = {
          id: Date.now(),
          text: `Claude Sonnet 4 has been ${this.claudeSonnet4Enabled ? 'enabled' : 'disabled'} for all clients.`,
          isUser: false,
          timestamp: new Date()
        };
      ...
      error: (error) => {
        console.error('Error updating Claude setting:', error);
        // Revert the checkbox on error
        this.claudeSonnet4Enabled = !this.claudeSonnet4Enabled;
      }
    });
  }
  ...
  checkDatabaseConnection() {
    this.http.get('http://localhost:3001/health').subscribe({
      next: (response: any) => {
        this.dbConnected = response.database === 'connected';
      },
      error: (error) => {
